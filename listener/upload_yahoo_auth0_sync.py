...
from common_configs.aws_storage import AwsStorage, STORAGE_USER_BUCKET, STORAGE_YAHOO_AUTH0_SYNC_FOLDER
from common_configs.listener.resource import Resource
from common_configs.message_bus import MessageBus
from common_configs.queue import UPLOAD_YAHOO_AUTH0_SYNC_BATCH
...
class IngestYahooAuth0Sync(Resource):
    batch_size = 5
    storage = AwsStorage()
    ...
    def listen(self, ch, method, properties, body):
        ...
        if RABBIT_INGESTION_REPLICAS:
            routing_key = self._get_queue_replica_topic(RABBIT_INGESTION_REPLICAS,
                                                        UPLOAD_YAHOO_AUTH0_SYNC_BATCH,
                                                        SVC_INGESTION_QUEUE)
        else:
            routing_key = UPLOAD_YAHOO_AUTH0_SYNC_BATCH
